  ...
  return {
    httpError: function (response) {
      ...
      if (response.data) {
        ...
        if (!errorSource.type && !errorSource.data && !errorSource.message && !errorSource.reason) {
...
angular.module('hbpCommon').service('hbpUtil', [
  ...
  function ($q, $http, hbpErrorService) {
    ...
    /**
     * Return an instance of ResultSet, which handle a pagination requests results.
     *
     * @param  {Object} res     an HTTPResponse or a promise
     * @param  {Object} options accept nextUrlKey, previousUrlKey, resultKey
     * @return {ResultSet}      an new instance of ResultSet
     */
    function paginatedResultSet(res, options) {
      return new ResultSet(res, options);
    }
    /**
     * Build a result set with internal support for fetching next and previous results.
     *
     * @param {Object} res the angular.$http Response
     */
    function ResultSet(pRes, options) {
      ...
      self.promise = $q.when(pRes).then(initialize).catch(handleError);
      function enqueue(fn) {
        self.promise = $q.when(self.promise.then(fn)).catch(handleError);
        return self.promise;
      }
      ...
      function next() {
        return enqueue(function () {
          return $http.get(self.nextUrl).then(handleNextResults);
        });
      }
      /**
      ...
      function previous() {
        return enqueue(function () {
          return $http.get(self.previousUrl).then(handlePreviousResults);
        });
      }
